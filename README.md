...
本地多模型图片分析。首页不区分文生图和图生图：文字与最多八张参考图进入同一个
创作总监对话，经方向、brief、模型选择和十三块审核后进入工作台。
首页会持续标出“说想法 → 选方向 → 确认简报 → 选模型 → 拆提示词”的当前位置和
下一步；导演请求同时显示阶段、耗时和失败位置，避免点击后无法判断是否仍在推理。
...
存为项目模板并再次带入，模板同时保存启用块集合和参数预设。画师收藏支持勾选混权重生成画师块；英文结构块输入内置 Tag
联想补全。中文局部修改采用“预览差异 → 确认 → 新版本”，撤销也追加为新版本。
创作总监链路中，确认简报原文会作为最终提示词锚点保留，模型适配英文只能附加，不能
替代已确认的画面事实；进入工作台的“仅拆解”输入也会载入完整简报事实，而非只取摘要。
LoRA-lite 支持显式登记原始 HTTPS 链接、版本、触发词、建议权重、兼容模型与冲突，
选择会作为不可变快照进入 Recipe；它不会扫描或操作本地 LoRA 文件。
PNG 出图检查可读取 A1111/ComfyUI 元数据、记录人工确认的本机实测，并可把结果导入按
SHA-256 去重的受管理本地资产；这些记录不自动触发出图或提升模型就绪状态。
文本生成可由前端停止等待；本地视觉 worker 另有服务端任务 ID 与真实取消，无法证明中断
计算的共享 HTTP 推理提供者会明确标为不支持真取消。快捷键：`Ctrl+Enter` 拓展、`Ctrl+Shift+Enter`
应用修改、`Ctrl+S` 保存作品、`Esc` 关闭弹层。

9 类、471 条词库已经接入确定性 `random plan` 和 Seed 恢复，但仍受实验开关保护：
目录尚未完成逐条语义审核，不能描述为公开发布就绪；已保留不可变 catalog 归档与十三块原生映射。工作台还提供全库/单项目逻辑 ZIP
...
`.prompt-studio-recovery/` 生成人工故障回滚副本，再迁移到当前 schema v3。该副本可能包含
本机 API Key，只用于迁移故障时由用户手动回滚，不可分享，也不是产品备份。

产品备份是经过 manifest/Hash 校验的逻辑 ZIP，包含安全的 LoRA-lite 元数据，但排除
密钥、模型/LoRA 文件和二进制资产；
