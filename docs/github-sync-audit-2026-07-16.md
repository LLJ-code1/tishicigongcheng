...
本报告记录差异和决策，不表示这些代码修复已经实施。本轮没有合并远端，也没有
修改业务代码、真实密钥、主数据库或模型文件。
...
审计过程中只执行了 `fetch` 和只读比较，没有执行 `pull`、`merge`、`reset`、提交或
推送。测试使用临时工作树、临时数据库、假密钥和 stub 引擎；没有向外部模型发起
请求。
...
Python 标准库服务器。AnimaDex 使用项目自己的 Python 虚拟环境。
...
| 去重口径前后端不同 | 前端 `toLowerCase` 无法合并 `Straße/STRASSE`、组合/分解重音；后端使用 `casefold` | 统一 NFKC + 明确 canonicalization，并用跨语言 golden fixtures 验证 | 预览、复制和运行时不会得到不同去重结果 |
...
- 文档中的“配方”应区分“结构块配方”和“完整生成配方”，避免把当前能力写成已满足
  PRD。
- `docs/architecture.md` 声称采用 casefold 去重，但前端实际只做 lowercase；应在修复
  代码时同步文档。
...
- 新 clone 按文档可启动；端口、健康检查、PID/停止流程和可选依赖行为有明确说明。
- 数据库迁移、备份、项目保存/重开/恢复经过实际验收。
