...
2. 与创作总监确认方向和 brief，选择基础模型并审核十三块适配结果后进入工作台。
3. 也可在工作台输入中文、英文或混合提示词，点击“拓展并结构化”，得到中英文 V1。
4. 在中间十三块修改动作、服装等内容，或点击“重生成”让当前项基于其他结构块重新
   生成。也可以在中文指令栏输入受支持的局部修改，先检查差异与锁定范围再确认。
5. 锁定已经确定的结构块，可阻止它被再次生成；锁定不等于应用修改。
6. 点击“应用修改（待保存）”，右侧输出更新但仍为未保存状态；再点击“保存作品”，
   服务端才创建连续的新版本。
7. 点击结构块中的“保存为资源”，或从快速资源处“新建”，可在本次页面会话的“我的
   素材”中复用；这两个入口尚未持久化。编辑器顶部“存为配方”会通过 favorites API
   持久化整组结构块。
8. “参考库”目前仍是界面入口，带图片、标签与检索的持久参考库尚未上线。
9. 进入“图片解析”，选择 WD14、Florence、JoyCaption 或 QwenVL，点击“开始多模型分析”。
